...
impl Deref for Bytes {
    ...
    fn deref(&self) -> &Self::Target {
        &self.inner[self.start.load(Ordering::Relaxed)..self.end]
    }
}

impl DerefMut for Bytes {
    fn deref_mut(&mut self) -> &mut Self::Target {
        // Since we have &mut self, we know we have exclusive access
        let start = *self.start.get_mut();
        // SAFETY: We have exclusive access via &mut self
        unsafe {
            let ptr = self.inner.as_ptr() as *mut u8;
            std::slice::from_raw_parts_mut(ptr.add(start), self.end - start)
        }
    }
}
...
impl Bytes {
    ...
    pub fn split(&self, size: usize) -> Result<Bytes, usize> {
        ...
        loop {
            ...
            if let Err(v) = self.start.compare_exchange_weak(
                start, 
                new_start, 
                Ordering::Relaxed, 
                Ordering::Relaxed
