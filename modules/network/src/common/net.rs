...
pub mod protocol {
    ...
    pub const BROADCAST_TEXT: u8 = 0x0D;
    ...
    pub enum TcpMsg<'a> {
        ...
        // Broadcast text message: (sender_name, text)
        BroadcastText(String, String)
    }
    ...
    impl TcpMsg<'_> {
        ...
        pub fn broadcast_text_encode<K: Cipher>(
            key: &K,
            nonce: u16,
            sender_name: &str,
            text: &str,
            ...
            out[0..2].copy_from_slice(&nonce.to_be_bytes());
            out[2] = MAGIC_NUM;
            out[3] = BROADCAST_TEXT;

            let msg_data = (sender_name.to_string(), text.to_string());
            let size = bincode::encode_into_slice(
                &msg_data,
                &mut out[TCP_MSG_HEADER_LEN..],
                config::standard(),
        ...
        fn decode(mode: u8, data: &[u8]) -> Result<TcpMsg<'_>> {
            let msg = match mode {
                ...
                BROADCAST_TEXT => {
                    let (msg_data, _) = bincode::decode_from_slice::<(String, String), _>(
                        data,
                        config::standard(),
                    )?;
                    TcpMsg::BroadcastText(msg_data.0, msg_data.1)
                }
    ...
    impl UdpMsg<'_> {
        ...
        pub fn broadcast_text_encode<K: Cipher>(
            ...
            let size = bincode::encode_into_slice(
                &msg_data,
                &mut out[UDP_MSG_HEADER_LEN..],
                config::standard(),
            )?;

            let ret = UDP_MSG_HEADER_LEN + size;
            let ctx = CipherContext {
                offset: 0,
                nonce
            };
        ...
        pub fn decode<'a, K: Cipher>(
            ...
            match mode {
                ...
                BROADCAST_TEXT => {
                    ...
                    Ok(UdpMsg::BroadcastText(msg_data.0, msg_data.1))
                }
