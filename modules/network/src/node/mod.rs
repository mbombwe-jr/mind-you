...
// Stable-compatible replacement for SyncUnsafeCell
#[repr(transparent)]
...
impl <K> From<&Interface<K>> for InterfaceInfo {
    fn from(value: &Interface<K>) -> Self {
        InterfaceInfo {
            ...
            has_udp_socket: value.udp_socket.is_some(),
        }
    }
}
                ...
                loop {
                    ...
                    if let Ok(packet) = UdpMsg::decode(key, packet) {
                        match packet {
                            ...
                            UdpMsg::BroadcastText(sender_name, text) => {
                                let group_name = interface.group_name.load();
                                let group_name_str = group_name.as_ref().map(|n| n.as_str()).unwrap_or("unknown");
                                info!("[GROUP {}] {}: {}", group_name_str, sender_name, text);
                            }
                        }
                    }
                }
                ...
                if !guard.is_empty() {
                    ...
                    for host in &*guard {
                        ...
                        if let Err(e) = update_hosts(&hb) {
                            // Check if it's a permission error (Windows error 5)
                            let is_permission_error = e.raw_os_error() == Some(5) || 
                                                     e.to_string().contains("Access is denied") ||
                                                     e.to_string().contains("Permission denied");
                            
                            if is_permission_error {
                                warn!("failed to clear hosts file entry '{}': {}. This requires administrator privileges on Windows.", host, e);
                            } else {
                            ...
                            loop {
                                ...
                                match msg {
                                    TcpMsg::NodeMap(map) => {
                                        ...
                                        #[cfg(any(target_os = "windows", target_os = "linux", target_os = "macos"))]
                                        if !config.features.disable_hosts_operation {
                                            let host_key = format!("TANROADS-{}", group_info.name);
                                            ...
                                            tokio::task::spawn_blocking(move || {
                                                ...
                                                match update_hosts(&hb) {
                                                    ...
                                                    Err(e) => {
                                                        ...
                                                        if is_permission_error {
                                                            warn!("node {} cannot update hosts file: {}. This requires administrator privileges on Windows. Network functionality will continue to work, but hostname resolution may not be available. You can disable hosts operation in config by setting 'disable_hosts_operation: true'", node_name, e);
                                                        } else {
                                                            error!("node {} update hosts error: {}", node_name, e);
                                                        }
                                                    }
                                                }
                                            });
                                    ...
                                    TcpMsg::BroadcastText(sender_name, text) => {
                                        info!("[GROUP {}] {}: {}", group_info.name, sender_name, text);
                                    }
                                    _ => continue,
                                }
                            }
