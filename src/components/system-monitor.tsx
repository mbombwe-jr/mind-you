...
const Sparkline = ({
  ...
  if (!data || data.length < 2) return null; // need at least 2 points to draw a path
  const pathRef = useRef<SVGPathElement>(null)

  const safeDenominator = Math.max(1, data.length - 1);
  const points = data.map((point, index) => ({
    x: (index / safeDenominator) * width,
...
export default function SystemMonitor({ compact }: SystemMonitorProps) {
  ...
  const [networkPoints, setNetworkPoints] = useState<DataPoint[]>([]);
  const { networkStats, refreshNetworkStats } = useSite();

  const [agents] = useState<Agent[]>([
    { id: "1", name: "Web Agent", memory: [], color: "#3b82f6" },
    { id: "2", name: "AI Assistant", memory: [], color: "#10b981" },
    { id: "3", name: "Background Sync", memory: [], color: "#f59e0b" },
    { id: "4", name: "Media Processor", memory: [], color: "#8b5cf6" },
  ])
  ...
  useEffect(() => {
    setNetworkPoints((prev) => {
      const next = [...prev, generateDataPoint(25, 40, 0.1)];
  ...
  // Format bits per second for network throughput (Kbps, Mbps, Gbps)
  const formatBits = (bytes: number): string => {
    const bits = bytes * 8;
  ...
  // Get only the format (unit) for a given number of bytes
  const getBitsFormat = (bytes: number): string => {
    const bits = bytes * 8;
    if (bits < 1000) return "bps";
    if (bits < 1_000_000) return "Kbps";
    if (bits < 1_000_000_000) return "Mbps";
    return "Gbps";
  };

  const txBytes = networkStats?.tx_bytes ?? 0;
  const rxBytes = networkStats?.rx_bytes ?? 0;
  const txFormat = getBitsFormat(txBytes);
  const rxFormat = getBitsFormat(rxBytes);
  let bgColor = '';
  if (txFormat === 'Gbps' || rxFormat === 'Gbps') {
    bgColor = 'bg-green-600';
  } else if (txFormat === 'Mbps' || rxFormat === 'Mbps') {
  ...
  if (compact) {
    // Only show network monitor (TX/RX) in compact mode
    return (
      <div className={`flex sidebar-3d-icon items-center border border-black/50 text-white px-2 py-1 rounded-md max-w-[170px] ${bgColor}`}>
        <div className="flex items-center mr-2">
          <Sparkline data={networkPoints} color="#fff" width={40} height={16} />
        </div>
        <div className="flex flex-col justify-center text-xs text-left min-w-[57px]">
          <div>TX: {formatBits(txBytes)}</div>
          <div>RX: {formatBits(rxBytes)}</div>
        </div>
      </div>
    );
  }
