...
/**
 * Modern Watch component with analog and digital display
 * Features: Real-time clock, analog hands, digital display, minute marks
 */
const watchCss = `
#watch { font-size:62.5%; margin:1em; }
...
const Watch: React.FC = () => {
  ...
  useEffect(() => {
    // Include the static CSS only once per mount
    if (!styleTagRef.current) {
      const styleTag = document.createElement('style');
      styleTag.innerHTML = watchCss;
  ...
  const minuteMarks = Array.from({ length: 48 }, (_, i) => <li key={i}></li>);

  // Generate hour digits
  const hourDigits = Array.from({ length: 12 }, (_, i) => <li key={i}>{i + 1}</li>);

  // Generate digital display numbers (always two digits)
  const pad2 = (n: number) => n.toString().padStart(2, '0');
  const hourNumbers = Array.from({ length: 24 }, (_, i) => <li key={i}>{pad2(formatHour(time))} </li>);
  const minuteNumbers = Array.from({ length: 60 }, (_, i) => <li key={i}>{pad2(formatMinute(time))}</li>);
  const secondNumbers = Array.from({ length: 60 }, (_, i) => <li key={i}>{pad2(formatSecond(time))}</li>);
  ...
  return (
    <div id="watch">
      ...
      <div className="digital-wrap">
        ...
        <ul className="digit-seconds" style={{ transition: 'transform 0.25s cubic-bezier(0.4, 2.2, 0.2, 1)', transform: `translateY(-${formatSecond(time) * 3}em)` }}>
          {secondNumbers}
        </ul>
      </div>
      <ul className="digits">
